  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignup && !emailSubmitted) {
        ...
        setMessage("Email sent! Check your inbox for the code.");
        return;
      }

      if (isSignup && emailSubmitted) {
        const res = await fetch("https://the-backend-production.up.railway.app/api/auth/email-otp/verify-email", {
        // const res = await fetch("http://localhost:3000/api/auth/email-otp/verify-email", {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
